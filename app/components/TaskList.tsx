...
function TaskCard({ task, incidentId, isTeamMember }: { task: Task, incidentId: string, isTeamMember: boolean }) {
  ...
  return (
    <div className="border border-gray-800 bg-gray-900 rounded-2xl p-3 space-y-3 hover:border-gray-700 transition-colors">
      <div className="flex justify-between items-start gap-2">
        <h4 className="font-medium text-gray-200 text-sm leading-tight">{task.title}</h4>
        <span className={`console-label text-[10px] px-2 py-0.5 rounded-full font-semibold shrink-0 ${statusColors[task.status]}`}>
      ...
      {task.assigneeIds.length > 0 ? (
        <div className="text-[10px] text-gray-500 flex gap-1 flex-wrap items-center">
          <span className="console-label text-gray-400">Assigned</span>
          {task.assigneeIds.map(id => (
            <span key={id} className="font-mono bg-gray-800 px-1.5 py-0.5 rounded text-gray-300">
              WORKER-{id.slice(0, 4).toUpperCase()}
            </span>
          ))}
        </div>
      ) : (
        <p className="console-label text-[10px] text-gray-600 italic">Unassigned</p>
      )}
      ...
      {isTeamMember && (
        <form onSubmit={handleCreateTask} className="flex gap-2">
          <input
            ...
            className="flex-1 min-w-0 bg-[var(--bg)] border border-gray-700 rounded-full px-3.5 py-2 text-sm text-gray-200 focus:outline-none focus:border-blue-500"
          />
          <button type="submit" disabled={!newTaskTitle.trim()} className="bg-[var(--ink)] hover:opacity-85 disabled:opacity-50 text-white rounded-full px-4 py-2 text-sm font-semibold transition-opacity shrink-0">
            Add
          </button>
        </form>
      )}
